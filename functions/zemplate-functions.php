...
function my_password_form() {
    ...
    $o = '<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" method="post" class="pass-page">
    <label for="' . $label . '">' . __( "Password Required:<br>" ) . ' </label><input class="password__box" name="post_password" id="' . $label . '" type="password" size="20" maxlength="20" /><br><input type="submit" name="Submit" value="' . esc_attr__( "Submit" ) . '" />
    </form>';
    return $o;
}
...
function excerpt_read_more_link($output) {
    global $post;
    return $output . ' <a href="'. get_permalink() . '">' . __('&hellip;') . '</a>';
}
